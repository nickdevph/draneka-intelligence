...
Use external research only when the execution policy permits it and it materially improves the answer. Prefer primary, authoritative, or well-established specialist sources. Record the source in provenance.
...
Material findings and hypotheses should cite evidence identifiers supplied in the execution context or locally assigned stable references within the result. References should allow a reviewer to determine whether a conclusion came from:
